...
[SETTINGS.md](SETTINGS.md)). Currently-muted repos are skipped silently, and so are repos
changed more recently than the optional `min_modified_age` setting allows.
...
| `m` | Mute this repo, then pick a timeframe (`1d` / `1w` / `1m`, or custom like `4h` / `3d` / `2w`). The repo is silently skipped in future `--commit-ask` runs until the mute expires (`1m` = 30 days). Returns to the loop (moves to the next repo). |
...
- [SETTINGS.md](SETTINGS.md) — the `commit_command` and `min_modified_age` keys.
