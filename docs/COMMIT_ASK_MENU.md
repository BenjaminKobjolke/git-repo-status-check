...
[SETTINGS.md](SETTINGS.md)). Currently-muted repos are not prompted for, and neither are
repos changed more recently than the optional `min_modified_age` setting allows. Both are
still listed in the report with a `[muted for 2 days]` / `[changed 12 minutes ago]` label,
and neither counts against `--limit`.
